...
module.exports = {
    ...
    gerar: function (caminho, nome, estrutura) {
        ...
        stream.once('open', function (fd) {
            ...
            stream.write(`\tapp.get('/api/${nome}/', controller.findAll);\n`);
            stream.write(`\tapp.get('/api/${nome}/:id', controller.findOne);\n`);
            ...
            for (var i = 0; i < estrutura.length; i++) {
                ...
                if(estrutura[i].props) {
                    if(estrutura[i].props[0] === "filtravel") {
                        ...
                        stream.write(`\tapp.get('/api/${nome}/findby/${item.nome}/:${item.nome}', controller.findBy${nomeFuncao});\n`);
                    }
                }                
            }
            

            stream.write(`\tapp.post('/api/${nome}/', controller.create);\n`);
            stream.write(`\tapp.put('/api/${nome}/:id', controller.update);\n`);
            stream.write(`\tapp.delete('/api/${nome}/:id', controller.delete);\n`);
            stream.write(`\n\n`);
            stream.write(`/*custom*/`);
